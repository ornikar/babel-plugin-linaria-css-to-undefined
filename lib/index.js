  ...
  return {
    ...
    visitor: {
      ...
      VariableDeclarator({ node, scope }, { file }) {
        ...
        const isValidStyledCallExpression = (callExpression) => {
        ...
        if (types.isTaggedTemplateExpression(node.init)) {
          ...
          if (types.isIdentifier(tag)) {
            ...
            if (!isValidStyledCallExpression(tag)) return;
            node.init = types.identifier('undefined');
          }
        }
        ...
        if (
          ...
          if (types.isMemberExpression(node.init.arguments[0].tag)) {
            if (!isValidStyledMemberExpression(node.init.arguments[0].tag)) return;
            node.init = types.identifier('undefined');
          } else if (types.isCallExpression(node.init.arguments[0].tag)) {
            if (!isValidStyledCallExpression(node.init.arguments[0].tag)) return;
            node.init = types.identifier('undefined');
          }
        }
      },
